...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="inner-page contact-page"><section className="contact-hero"><p className="eyebrow">{t.contactEyebrow}</p><h1>{t.contactTitle}</h1><p>{t.contactIntro}</p><a className="big-contact-link" href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"><span>{t.whatsapp}</span><i aria-hidden="true">↗</i></a></section>
