...
export default async function StudioPage({ params }: { params: Promise<{ locale: string }> }) { const { locale } = await params; if (!isLocale(locale)) notFound(); const t = copy[locale]; return <main className="inner-page studio-page"><section><p className="eyebrow">Sanity CMS</p><h1>{t.studioTitle}</h1><p>{t.studioText}</p><div><a className="solid-button" href={sanityStudioUrl} target="_blank" rel="noopener noreferrer">{t.studioOpen} ↗</a><a className="text-link" href="/README.md">{t.studioSetup} ↗</a></div></section></main>; }
